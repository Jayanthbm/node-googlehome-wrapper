...
function App() {
  ...
  const FanComponent = ({ data, forceReload }) => {
    ...
    const [debouncedSpeed, setDebouncedSpeed] = useState(currentLevel);
    ...
    const handleRefresh = async () => {
      ...
      localStorage.setItem(`${data.deviceName}-status`, JSON.stringify(deviceStatus));
      setLastFetchedAt(deviceStatus["lastFetchedAt"]);
      setLoading(false);
    };
  ...
  useEffect(() => {
    const updateSpeed = async () => {
      console.log("update speed function", debouncedSpeed, currentLevel);
      if (debouncedSpeed !== currentLevel) {
        ...
        setCurrentLevel(debouncedSpeed);
        setLastFetchedAt(deviceStatus["lastFetchedAt"]);
        setLoading(false);
      }
    };
    ...
    return (
      <div className="wrapper">
        ...
        <div className="info">Last Updated: {new Date(lastFetchedAt).toLocaleString()}</div>
      </div>
    );
  };
